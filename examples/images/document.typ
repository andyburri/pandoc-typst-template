...
#figure(```markdown
`#outline(target: figure.where(kind: image))`{=typst}
```
)
= Image with caption `{width=75%}`
<image-with-caption-width75>
#figure(image("image.jpg", width: 75.0%),
  caption: [
    This is the caption
  ]
)

= Left Aligned Image `{align=left width=30%}`
<left-aligned-image-alignleft-width30>
#[
#show figure: set align(left)
#figure(image("image.jpg", width: 30.0%),
  caption: [
    Left aligned image
  ]
)
...
= Right Aligned Image `{align=right width=45%}`
<right-aligned-image-alignright-width45>
#[
#show figure: set align(right)
#figure(image("image.jpg", width: 45.0%),
  caption: [
    Right aligned image
  ]
)
...
<image-with-no-caption>
#box(image("image.jpg", width: 40.0%))

= Inline Image `{height=1.5em}`
<inline-image-height1.5em>
Some text before the #box(image("image.jpg", height: 1.5em)) and some
more text after it.

#figure(image("image.jpg", width: 25.0%),
  caption: [
    Another image
  ]
)
